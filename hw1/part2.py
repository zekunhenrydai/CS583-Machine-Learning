...
class Tree(object):
    ...
    def conditional_entropy(Y,X):
        ...
        ce = 0.0
        n = len(X)
        labelCounts_x = Counter(X)
        condition_set = {}
        for i in range(n):
            feature = X[i]
            if feature not in condition_set:
                condition_set[feature] = []
            condition_set[feature].append(Y[i])
        
        for key in condition_set:
            if key in labelCounts_x:
                value = labelCounts_x[key]
            p_x = float(value)/n
            ce += p_x * Tree.entropy(condition_set[key])
        #########################################
        return ce 
    ...
    def best_attribute(X,Y):
        ...
        for index in range(p):
            ...
            for instance in range(n):
                features.append(X[index][instance])
                g = Tree.information_gain(Y,features)
            if g > best_attribute:
                i = index
                best_attribute = g
    ...
    def split(X,Y,i):
        ...
        ## INSERT YOUR CODE HERE
        C = dict()
        chosen_feature = Counter(X[i])
        n = len(chosen_feature)

        
        for key in chosen_feature:
            X_split = []
            Y_split = []
            for p in range(len(X)):
                ...
                for index in range(len(X[i])):
                    if X[i][index] == key:
                        X_temp.append(X[p][index])
                X_split.append(X_temp)
            Y_temp = []
            for index in range(len(X[i])):
                if X[i][index] == key:
            ...
            #print(Y_split)
            childNode = Node(X_split,Y_split)
            C[key] = childNode
    ...
    def stop2(X):
        ...
        row_counts = []
        for p in range(len(X)):
            labelCounts = Counter(X[p])
            # print(labelCounts)
            if len(labelCounts) == 1:
                row_counts.append(True)
            else:
                row_counts.append(False)
        res = Counter(row_counts)
        if len(res) == 1 and res[1] == len(X):
            s = True
        #########################################
        return s
    ...
    def most_common(Y):
        ...
        ## INSERT YOUR CODE HERE
        labelCounts = Counter(Y)
        y = labelCounts.most_common(1)[0][0]
        #########################################
        return y
    ...
    def train(X, Y):
        ...
        p = Y[0]
        C = Tree.split(X,Y,i)
        t = Node(X, Y, i, C, False, p)
        print("##Build Tree start")
    ...
    def predict(t,X):
        ...
        for p in range(0,len(X[0])):
            ...
            for n in range(len(X)):
                X_column.append(str(X[n][p]))
            result = Tree.inference(t,np.array(X_column))
            Y.append(result)
        Y = np.array(Y)
        print(Y)
...
if __name__ == "__main__":
    ...
    #rint(X)
    data1 = [['Tom','Ana'],['low','low'],['low','medium'],['yes','yes'],['male','female']]
    #print[data1]
    Y_predict = Tree.predict(t,data1)
